  ...
  >
    <v-card rounded="lg">
      <v-card-title class="d-flex align-center ga-3 pt-6 px-7 pb-0">
        ...
        <span class="text-h6 font-weight-bold">{{ t('requesters.deleteRequester') }}</span>
      </v-card-title>

      <v-card-text class="px-7 pt-4 pb-2">
        <p class="text-body-2 mb-2">
          {{ t('requesters.confirmDelete', { name: requester?.name ?? '' }) }}
        </p>
        <p class="text-caption text-medium-emphasis">
          {{ t('requesters.confirmDeleteDescription') }}
        </p>
      </v-card-text>
