  ...
  >
    <v-card rounded="lg">
      <v-card-title class="d-flex align-center ga-3 pt-6 px-7 pb-0">
        ...
        <span class="text-h6 font-weight-bold">{{ t('warehouses.deleteWarehouse') }}</span>
      </v-card-title>

      <v-card-text class="px-7 pt-4 pb-2">
        <p class="text-body-2 mb-2">
          {{ t('warehouses.confirmDelete', { name: warehouse?.name ?? '' }) }}
        </p>
        <p class="text-caption text-medium-emphasis">
          {{ t('warehouses.confirmDeleteDescription') }}
        </p>
      </v-card-text>
